...
"""
========================  VERSIÓN DEMO / PILOTO  ========================
Versión ABREVIADA para demostración (≈4-5 min). Conserva TODAS las fases del
experimento real pero con bloques cortos y menos repeticiones, para recorrer
el flujo completo sin correr la sesión entera. NO usar para recolectar datos.
Truco: ingresa un ID PAR para ver el grupo de contracondicionamiento, o un ID
IMPAR para ver el grupo de habituación.
=========================================================================
...
FASES
-----
  (DEMO: duraciones y repeticiones reducidas — ver PARÁMETROS)
  1. Entrenamiento    — 1 sesión × 2 bloques × 25 s (q y p → fichas, RI corto)
  2. Castigo          — 1 sesión × 2 bloques × 25 s (una tecla castigada con
                        sonido en RF individualizado)
  3. Revalorización   — 4 eventos (según grupo):
                          · Contracondicionamiento: sonido → puntos
                          · Habituación: sonido solo, repetido
  4. Prueba extinción — 1 bloque × 25 s (fichas siguen; SIN sonido)
  5. Prueba reforzada — 1 bloque × 25 s (vuelve el sonido, mismas contingencias)
  6. Cierre
...
#  PARÁMETROS
# ═══════════════════════════════════════════════════════════════
TRAIN_DURATION   = 25    # s por bloque de entrenamiento   (DEMO; real=120)
PUNISH_DURATION  = 25    # s por bloque de castigo          (DEMO; real=120)
TEST_DURATION    = 25    # s por bloque de prueba           (DEMO; real=120)

N_TRAIN_SESSIONS        = 1  # sesiones de entrenamiento           (DEMO; real=1)
N_TRAIN_BLOCKS_SESSION  = 2  # bloques por sesión de entrenamiento (DEMO; real=6)
N_PUNISH_SESSIONS       = 1  # sesiones de castigo                 (DEMO; real=2)
N_PUNISH_BLOCKS_SESSION = 2  # bloques por sesión de castigo       (DEMO; real=3)
N_TEST_BLOCKS          = 1   # bloques por prueba (extinción y reforzada)

RI_T             = 6.0   # media del intervalo RI de fichas (DEMO; real=15.0)
TARGET_SOUNDS    = 5     # sonidos objetivo por bloque de castigo (DEMO; real=10)
N_REVAL_EVENTS   = 4     # eventos de revalorización por grupo    (DEMO; real=20)
...
# ═══════════════════════════════════════════════════════════════
dlg = gui.Dlg(title='Experimento de Aprendizaje (Castigo) — DEMO')
...
DATA_PATH = os.path.join('data', f'DEMO_castigo_sub_{PARTICIPANT_ID}_{_ts}.csv')
...
# ── Bienvenida ────────────────────────────────────────────────
show_msg(
    'VERSIÓN DEMO (abreviada)\n\n'
    'Esta es una demostración corta (~4-5 min) que recorre TODAS las fases\n'
    'del experimento con bloques breves. No sirve para recolectar datos.\n\n'
    f'Grupo asignado a este ID ({PARTICIPANT_ID}): '
    f'{"CONTRACONDICIONAMIENTO" if GROUP == "contracond" else "HABITUACIÓN"}.\n'
    'Prueba con un ID par y uno impar para ver ambos grupos.\n\n'
